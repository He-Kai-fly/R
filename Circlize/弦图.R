#!/usr/bin/env Rscript
rm(list = ls())
setwd("E:\\Study\\Bioinformation\\Test\\Script\\Circlize")
##加载R包，初始传递命令
...
otu_table_file <- 'otu_table_16s.txt'
group_file <- 'group_16s.txt'
taxonomy_file <- 'taxonomy_16s.txt'
...
color_sample <- c('#6181BD', '#F34800', '#64A10E', '#FF00FF', '#c7475b', '#049a0b', '#386CB0', '#F0027F','#CC99FF','#99CC00','#9999FF','#CC9966')
color_phylum <- c('#BEAED4', '#FDC086', '#FFFF99')
color_group <- c('#ff4308','#4253ff')
...
####circlize 绘图
pdf('HEKAI1.pdf', width = 20, height = 8)
